"""
Exercise 07: Diagnosing Prompt Failures & Structured Outputs (Module 2)
...
import json  # parse response JSON trả về từ Structured Outputs
...
MODEL = "claude-haiku-4-5"  # dùng haiku cho dev/test theo convention project (rẻ hơn sonnet/opus)
...
#     hệ thống system prompt này trước khi gửi request.)
FIXED_SYSTEM_PROMPT = (
...
def classify_bare(ticket: str) -> str:
    ...
    response = client.messages.create(
        model=MODEL,  # model dùng để classify
...
def classify_fixed(ticket: str) -> str:
    ...
    # phần instruction/guideline chính, đúng nguyên tắc đã học ở Exercise 06.
...
# CONTACT_SCHEMA: JSON schema mô tả chính xác field cần trích xuất. additionalProperties:
# False + required đầy đủ là bắt buộc để API compile được "grammar" ràng buộc decoding.
CONTACT_SCHEMA = {
    "type": "object",
    "properties": {
        "name": {"type": "string"},
...
def extract_contact_structured(text: str) -> dict:
    ...
    # output_config.format đảm bảo block đầu tiên là text chứa JSON hợp lệ theo schema —
    # nhưng vẫn phải check stop_reason trước khi tin response parse được (refusal/max_tokens
    # là 2 trường hợp response không khớp schema dù đã bật structured outputs).
...
# BOOK_FLIGHT_TOOL: tool có strict=True — argument Claude truyền vào (destination,
# date, passengers) sẽ được validate khớp input_schema TRƯỚC KHI code tự viết chạy,
# tránh trường hợp model gửi input sai kiểu/thiếu field làm crash hàm xử lý.
BOOK_FLIGHT_TOOL = {
    "name": "book_flight",
