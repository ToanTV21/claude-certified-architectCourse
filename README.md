...
| 02 | [sessions/02-prompt-evaluation](sessions/02-prompt-evaluation/notes.md) | Prompt Evaluation | 🔄 In progress | Quy trình **eval** một prompt: xây dựng **test dataset**, chạy eval, và hai cách chấm điểm — **model-based grading** (dùng LLM làm giám khảo) và **code-based grading** (chấm bằng logic code). |
| 03 | [sessions/03-prompt-engineering-techniques](sessions/03-prompt-engineering-techniques/notes.md) | Prompt Engineering Techniques | ⬜ Todo | Các kỹ thuật viết prompt hiệu quả: rõ ràng & trực tiếp (**clear and direct**), càng cụ thể càng tốt (**being specific**), cấu trúc prompt bằng **XML tags**, và kỹ thuật **few-shot** (đưa ví dụ mẫu). |
| 04 | [sessions/04-tool-use-with-claude](sessions/04-tool-use-with-claude/notes.md) | Tool Use with Claude | ⬜ Todo | Cho Claude gọi **tool/function**: định nghĩa **tool schema**, xử lý **message blocks**, gửi lại **tool results**, các vòng **multi-turn conversation** khi có tool, dùng nhiều tool cùng lúc, **fine-grained tool calling**, và 2 tool có sẵn — **text edit tool** và **web search tool**. |
...
| 07 | [sessions/07-model-context-protocol](sessions/07-model-context-protocol/notes.md) | Model Context Protocol | ⬜ Todo | Giới thiệu **MCP (Model Context Protocol)**: setup project, viết **MCP client**, định nghĩa **tools** qua MCP, dùng **server inspector** để debug, định nghĩa và truy cập **resources**, định nghĩa **prompts** và dùng chúng trong client. |
| 08 | [sessions/08-anthropic-apps-claude-code](sessions/08-anthropic-apps-claude-code/notes.md) | Anthropic Apps — Claude Code & Computer Use | ⬜ Todo | Các app do Anthropic xây dựng: setup và thực hành **Claude Code**, mở rộng khả năng của nó bằng cách kết nối thêm **MCP servers**. |
| 09 | [sessions/09-agents-and-workflows](sessions/09-agents-and-workflows/notes.md) | Agents and Workflows | ⬜ Todo | Các pattern xây **agentic workflow**: **parallelization**, **chaining**, **routing**, kết hợp agent với tool, kỹ thuật **environment inspection**, và so sánh **workflows vs agents** — khi nào dùng cái nào. |
| 10 | [sessions/10-final-assessment](sessions/10-final-assessment/notes.md) | Final Assessment | ⬜ Todo | Bài đánh giá tổng kết cuối khóa, ôn lại toàn bộ kiến thức đã học qua các session trước. |
